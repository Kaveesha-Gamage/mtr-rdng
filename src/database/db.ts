...
export const initDatabase = () => {
  ...
  try {
    db.execSync(`SELECT accountNumber, customerName, mobileNo, netType, netTypeName, hasReading FROM pending_readings LIMIT 0;`);
  } catch (error) {
  ...
  // 3. Create pending_readings table (matches /reading-status/area/{code}/pending API response)
  db.execSync(`
